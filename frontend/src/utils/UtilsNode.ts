const { dirname, basename, extname } = require("path") as typeof import("path");
const fs = require("fs") as typeof import("fs");
import { Stats, WriteFileOptions } from "fs";
import { fsPromises, path, rimraf } from "./UtilsExports";
import {
  MapEx,
  arrEx,
  canvasToBlob,
...
export const coverImgToWebp = async (
  file: File | File[],
  options?: CoverImgToWebpOptions
) => {
  if (Array.isArray(file)) return file.map((i) => coverImgToWebp(i, options));
  return new Promise(async (resolve, reject) => {
    const inputQuality = Number(options.quality);
    const { deleteOrigin, each, type: inputType, quality } = mergeObject(
      {
        deleteOrigin: false,
        type: "webp",
        quality: 1,
      ...
      {
        ...options,
        quality: isNaN(inputQuality) ? 1 : inputQuality,
      }
    );
    console.log(quality, "quality");
    const type = (inputType === "jpg" ? "jpeg" : inputType) as any;
    const blob = await fileToBlob(file);
    const map = await getCanvas(blob);
    const newBlob = await canvasToBlob(map.canvas, ("image/" + type) as any, {
      quality,
    });
    const { path } = file;
    const dir = getDirName(path);
    const name = getName(path);
    const arraybuffer = (await getResByFile(newBlob, "arrayBuffer")) as any;
    const buf = Buffer.from(arraybuffer);
    try {
      if (deleteOrigin) {
        fs.unlinkSync(path);
      }
      fs.writeFile(pJoin(dir, name + "." + inputType), buf, async (res) => {
        console.log(res, "res");
        if (!res) {
          each?.(file);
        }
        res ? reject() : resolve(file);
      });
    } catch (e) {
      reject(e);
    }
...
export const getExt = (dir: string, point = true) => {
  const str = path.extname(dir);
  return point ? str : rmPartOfString(str, ".");
};
...
export const eachFileForDir = (map: {
  dir: string;
  deep?: boolean;
  each: (fileName: string, info: Stats | null, nowDir: string) => void;
  done?: () => void;
}) => {
  return _eachFileForDir({
    ...map,
    doneMap: new MapEx(),
    trigger: fnDebounce(() => {
      console.log(79, "79");
...
export const analysisDir = (map: {
  dir: string;
  deep: boolean;
  ls?: Set<string>;
}) => {
  const inputMap = mergeObject(
    { ls: new Set<string>(), dir: "", deep: false },
    map
  );
...
export const moveDir = (oldPath: string, newPath: string): Promise<any> => {
  createDir(newPath);
  return fsPromises.readdir(oldPath).then(async (files) => {
    //遍历读取到的文件列表
      ...
      await Promise.all(
        files.map(async (filename) => {
          ...
          return new Promise(async (s) => {
            ...
            if (isFile) {
              // 通过重命名移动文件到根目录
              await fsPromises.rename(filedir, path.join(newPath, filename));
            } else {
...
export const isVideoPaths = (path: string | string[]) => {
  return arrEx
    .toArr(path)
    .filter((i) => supportVideoExt.includes(extname(i).toLocaleLowerCase()));
};
